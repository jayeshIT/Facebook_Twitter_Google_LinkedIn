...
send_button.addEventListener('click', function(e) {
	...
	Titanium.API.info('---Send to :' + Alloy.Globals.send_id_following);
	//viewMessage.zIndex = 0;
	//viewMessage.visible = false;
	BH.send_message('text=' + textArea_Mesage.value + '&user_id=' + Alloy.Globals.send_id_following, function(resp) {
...
$.TwitterMyFollowing.add(viewMessage);

function GetFriends(cursorData) {
	Titanium.API.info('------Cursor:------:' + cursorData);
	Ti.App.showIndicator();
	//BH.get_Friends('cursor=' + cursorData + 'screen_name=' + Ti.App.Properties.getString('ttname') + 'skip_status=' + true + 'include_user_entities=' + false, function(resp) {
	BH.get_Myfollowing('cursor=' + cursorData + '&screen_name=' + Ti.App.Properties.getString('ttname') + '&skip_status=' + true + '&include_user_entities=' + false, function(resp) {
		Titanium.API.info('----Resoponse user------' + JSON.parse(resp));
		try {
			if (resp != null && resp != "" && resp != false && resp != undefined && resp != 0) {
				...
				for (var i = 0; i < JSobj.users.length; i++) {
					...
					var rowFriends = Titanium.UI.createTableViewRow({
					...
					rowFriends.add(view_frame);
					rowFriends.filter = name_lbl.text;
					rowFriends.add(name_lbl);
					rowFriends.add(location_lbl);
					rowFriends.add(time_lbl);
					rowFriends.add(Message);
					if (cursor1 == -1) {
						Titanium.API.info('--push data -------');
						Friends.push(rowFriends);
					} else {
						Titanium.API.info('--Append row data -------');
						$.tableFriends.appendRow(rowFriends);
					}
				}
				if (cursor1 == -1) {
					Titanium.API.info('--Set DAta in TableView  -------');
					$.tableFriends.setData(Friends);
				}
				...
				Ti.App.hideIndicator();
				alert("Error in loading Friends..\nPlease try again.");
			}
		} catch(ex) {
			Ti.App.hideIndicator();
...
$.tableFriends.addEventListener('click', function(e) {
	if (e.source.cust_id == 'Message') {
		...
		Alloy.Globals.send_id_following = e.row.id;
	}
});
